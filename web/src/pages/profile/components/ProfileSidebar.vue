...
<template>
  <aside class="h-max min-w-80 p-5 rounded-lg bg-card text-card-foreground">
    ...
    <div :class="$style.actions">
      <template v-if="isEditMode">
        <button :class="[$style.actionsButton, $style.saveButton]" @click="emit('saveButtonClick')">Save</button>
        <button :class="[$style.actionsButton, $style.cancelButton]" @click="emit('cancelButtonClick')">Cancel</button>
      </template>
      ...
      <template v-else>
        ...
        <button v-else :class="[$style.actionsButton, $style.followButton]" @click="emit('followButtonClick')">
    ...
    <div :class="$style.followingBox">
      <i class="pi pi-users" :class="$style.followingIcon" />
    ...
    <div :class="$style.links">
      <template v-if="isEditMode">
        <div :class="$style.link">
          ...
          <InputText :class="$style.linkInput" />
        </div>
        <div :class="$style.link">
      ...
      <template v-else>
        <a
          ...
          target="_blank"
        >
          <component :is="icon" :class="$style.linkIcon" />
          <span>{{ label }}</span>
        </a>
      </template>
    </div>
